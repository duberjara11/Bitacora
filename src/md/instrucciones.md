Ya tengo el código fuente real. Nada que adivinar, nada de shadcn necesario.

Dato clave

El JSON solo pide una dependencia, ogl@^1.0.11. Cero dependencias de shadcn. Puedes cancelar esa instalación.

Paso 1: cancela el CLI de shadcn

Ctrl+C en esa terminal si sigue abierta.

Paso 2: instala la única dependencia real

bash
pnpm add ogl@^1.0.11

Paso 3: crea los archivos

Dentro de components/graph o donde vayas a usar el efecto, crea dos archivos.

RippleDistortion.css
...
RippleDistortion.jsx

El archivo es largo, es un shader WebGL completo. Lo pego tal cual vino del registro oficial, sin modificar nada.
...
const screenVertex = `precision highp float; attribute vec2 position; attribute vec2 uv; varying vec2 vUv; void main() {   vUv = uv;   gl_Position = vec4(position, 0.0, 1.0); }`;
...
Paso 4: permite archivos .jsx en el proyecto TypeScript

Abre tsconfig.app.json y agrega esto dentro de compilerOptions.

json
"allowJs": true

Sin esto, TypeScript puede marcar error al importar un archivo .jsx desde un .tsx.

Advertencia

Este componente usa window.Image y eventos globales de pointermove. En React con StrictMode puede montarse dos veces en desarrollo. No lo he probado en tu entorno, confírmalo al correrlo.
