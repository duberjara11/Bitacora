...
const RippleDistortion = ({
  ...
  configRef.current = { brushSize, spread, fade, spacing, clickStrength, trigger, enabled, auto };
  ...
  useEffect(() => {
    ...
    const loop = (now) => {
      ...
      if (hasActiveWaves) scheduleFrame();
    };
    ...
    scheduleFrame();
    let autoPosition = 0;
    const autoTimer = auto && !reduceMotion ? window.setInterval(() => {
      const cfg = configRef.current;
      const x = width * (0.5 + Math.sin(autoPosition * 0.83) * 0.34);
      const y = height * (0.5 + Math.cos(autoPosition * 0.61) * 0.34);
      autoPosition += 1;
      setNewWave(x, y, 0.65);
      scheduleFrame();
      if (!cfg.enabled) window.clearInterval(autoTimer);
    }, 2400) : undefined;
